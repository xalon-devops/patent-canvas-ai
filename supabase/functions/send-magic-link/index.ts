...
serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      logStep("Error generating magic link", { error: error.message });
      throw error;
    }
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="600" style="margin: 0 auto; background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                  ...
                  <tr>
                    <td style="padding: 40px;">
                      <h2 style="margin: 0 0 20px; color: #1a1a2e; font-size: 22px; text-align: center;">🔑 Your Magic Sign-In Link</h2>
                      
                      <p style="margin: 0 0 20px; color: #4a5568; font-size: 16px; line-height: 1.6; text-align: center;">
                        Click the button below to securely sign in to your ${APP_NAME} account. No password required!
                      </p>
                      ...
                      <div style="background-color: #fef3c7; border: 1px solid #f59e0b; border-radius: 8px; padding: 15px; margin: 25px 0;">
                        <p style="margin: 0; color: #92400e; font-size: 14px; line-height: 1.5;">
                          <strong>⚠️ Security Notice:</strong> This link will expire in 1 hour and can only be used once. If you didn't request this, please ignore this email.
                        </p>
                      </div>
                  ...
                  <tr>
                    <td style="background-color: #f8fafc; padding: 25px 40px; border-radius: 0 0 12px 12px; border-top: 1px solid #e2e8f0;">
                      <p style="margin: 0 0 10px; color: #64748b; font-size: 14px; text-align: center;">
                        Prefer using a password? You can set one in your account settings.
                      </p>
    ...
    return new Response(
      JSON.stringify({ success: true, message: "Magic link email sent" }),
      { 
        status: 200, 
    ...
    logStep("Error in send-magic-link function", { error: error.message });
    return new Response(
